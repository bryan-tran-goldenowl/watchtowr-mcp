    ...
    def get_watchtowr_source_ips() -> str:
        ...
        try:
            ...
            if isinstance(data, list):
                formatted = [f"• {ip}" for ip in data]
                return f"watchTowr Source IP Addresses ({len(formatted)}):\n" + "\n".join(formatted)
            return f"watchTowr Source IP Addresses: {data}"
        except Exception as e:
            return f"Error retrieving source IP addresses: {e}"

    @mcp.tool()
    def get_activity_logs(page_size: int = 10) -> str:
        ...
        try:
            ...
            response = api.get_list_activity_logs(page_size=min(page_size, 30))
    ...
    def get_business_unit_details(business_unit_id: int) -> str:
        ...
        try:
            ...
            lines = [
                ...
                f"Created: {getattr(bu, 'created_at', 'N/A')}",
            ]
            return "\n".join(lines)
        except Exception as e:
